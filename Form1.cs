...
namespace Samyang_135mm_Focuser
{
    public partial class Form1 : Form
    {
        ...
        private byte moveDirectionState = 0x03;
        ...
        private byte getNumberOfSteps(int multiplier, byte dir)
        {
            int backlash = backlashCorrection(dir);
            int numSteps = multiplier * Convert.ToInt32(lblStepSize.Text) + backlash;
            return Convert.ToByte(numSteps);
        }

        private int backlashCorrection(byte dir)
        {
            if (dir == moveDirectionState)
            {
                return 0;
            }
            else
            {
                moveDirectionState = dir;
                return Convert.ToInt32(backlashSteps.Text);
            }
        }
        ...
        {
            groupBox2.Enabled = false;
            byte numSteps = getNumberOfSteps(multiplier, dir);
